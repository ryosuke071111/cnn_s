...
from p_gen_vocab_from_file import Vocab,generateVocab
import os
...
from p_gen_attn_model import AttnEncoderDecoder
import sys
import  argparse
import math
...
mce = nn.NLLLoss(reduction='mean', ignore_index=PAD)
def masked_cross_entropy(logits, target):
    return mce(logits.view(-1, logits.size(-1)), target.view(-1))


def compute_loss(batch_X, batch_Y, lengths_X, model, max_oovs, oovs, articles_extend, abstracts_extend, optimizer=None, is_train=True):
    # 損失を計算する関数
    ...
    # 推論
    pred_Y = model(batch_X, lengths_X, max_length, max_oovs, oovs, articles_extend, abstracts_extend, batch_Y, use_teacher_forcing)
    # 損失関数を計算
    loss = masked_cross_entropy(pred_Y.contiguous(), abstracts_extend.contiguous())
    ...
    abstracts_extend = abstracts_extend.transpose(0,1).contiguous().data.cpu().tolist()
    pred = pred_Y.max(dim=-1)[1].data.cpu().numpy().T.tolist()
    return loss.item(), abstracts_extend, pred
...
def Train():
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for batch in data_loader:
            batch_X, batch_Y, lengths_X, max_oovs, oovs, articles_extend, abstracts_extend  = batch
        ...
        if epoch % 20 == 0:
            ...
            print('pred', vocab.outputids2words(pred[0], oovs[0]))
            print()
...
vocab = generateVocab()
articles, abstracts, oovs, articles_extend, abstracts_extend = data_generate(vocab, num_of_data)
data_loader = DataLoader(articles, abstracts, oovs, articles_extend, abstracts_extend, batch_size, shuffle=True)
